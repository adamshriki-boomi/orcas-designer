...
import type { VisualQaFinding, VisualQaReport } from '@/lib/types';
...
import { SideBySide } from '@/components/visual-qa/side-by-side';
import { FindingsList } from '@/components/visual-qa/findings-list';
...
export default function ReportDetailClient({ id: routeId }: ReportDetailClientProps) {
  ...
  const onFindingsChange = useCallback(
    async (next: VisualQaFinding[]) => {
      if (!report) return;
      const counts = computeSeverityCounts(next);
      setReport({ ...report, findings: next, severityCounts: counts });
      try {
        await updateReport(report.id, { findings: next, severityCounts: counts });
      } catch (e) {
    ...
    <FadeIn>
      ...
      <PageContainer>
        <div className="mx-auto max-w-5xl space-y-6">
          ...
          {showPublishForm && (
            <div className="rounded-lg border border-border bg-card p-4">
              ...
              <p className="mb-3 text-xs text-muted-foreground">
                ...
                screenshots attached and findings grouped by category.
              </p>
          ...
          <section>
            <h2 className="mb-2 text-base font-semibold">Findings</h2>
            <FindingsList findings={report.findings} onChange={onFindingsChange} />
          </section>
        </div>
      </PageContainer>
    </FadeIn>
