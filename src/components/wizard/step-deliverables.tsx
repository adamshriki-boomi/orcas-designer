...
/**
 * Deliverables & Constraints. The old "Output Type" and "Interaction Level"
 * pickers are gone — the AI always instructs Claude Code through the lo-fi →
 * animated prototype → hi-fi progression. This step only configures
 * constraints: where to write, what to support, optional design tokens.
 */
export function StepDeliverables({
  ...
  return (
    <WizardStep
      title="Deliverables & Constraints"
      description="Claude Code will always produce lo-fi → animated prototype → hi-fi. Configure where the output lives and what it must support."
    >
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Where Claude Code writes the three phases (e.g. <code>./output/phase1/</code>, <code>./output/phase2/</code>).
          </p>
        </div>
        ...
        <CollapsibleSection title="External resources" defaultOpen={!externalResourcesAccessible}>
          <div className="flex items-center gap-3">
            <Switch checked={externalResourcesAccessible} onCheckedChange={onExternalResourcesChange} />
            <Label>Are external URLs publicly accessible?</Label>
          </div>
          <p className="text-xs text-muted-foreground">
        ...
        <CollapsibleSection title="Design direction (optional)" defaultOpen={designDirection !== null}>
          <div className="space-y-4">
            <Input
