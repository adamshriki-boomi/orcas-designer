...
import { createTestPrompt, createPromptWithFigma, createPromptWithStorybookMemory } from '@/test/helpers/prompt-fixtures'
...
describe('buildOutputTypeSection', () => {
  it('includes "Static Mockups Only" for static interaction level', () => {
    const project = createTestPrompt({ interactionLevel: 'static' })
  ...
  it('includes Claude-to-Figma reference when figma target is set', () => {
    const project = createPromptWithFigma({ interactionLevel: 'static' })
    const result = buildOutputTypeSection(project)
