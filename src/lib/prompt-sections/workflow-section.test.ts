...
import { createTestPrompt, createPromptWithFigma, createPromptWithDesignSystem, createPromptWithCurrentImpl, createFullPrompt, createTestSharedSkill, createPromptWithStorybookMemory, createPromptWithPrototypeFigma } from '@/test/helpers/prompt-fixtures'
...
describe('buildWorkflowSection', () => {
  ...
  it('includes click-through flows step for click-through interaction level', () => {
    const project = createTestPrompt({ interactionLevel: 'click-through' })
    const result = buildWorkflowSection(project)
    expect(result).toContain('click-through flows')
  })

  it('includes interactive prototypes step for full-prototype interaction level', () => {
    const project = createTestPrompt({ interactionLevel: 'full-prototype' })
    const result = buildWorkflowSection(project)
  ...
  it('includes cross-browser verification for multiple browsers', () => {
    const project = createTestPrompt({ browserCompatibility: ['chrome', 'firefox', 'safari'] })
    const result = buildWorkflowSection(project)
  ...
  it('slugifies the project name for the git branch name', () => {
    const project = createTestPrompt({ name: 'My Cool Design Project' })
    const result = buildWorkflowSection(project)
  ...
  it('adds Figma extraction step when prototype URL is a Figma URL', () => {
    const project = createPromptWithPrototypeFigma()
    const result = buildWorkflowSection(project)
    expect(result).toContain('Figma MCP tools')
