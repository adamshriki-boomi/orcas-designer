...
function getFrameworkGuidance(methodId: string): string {
  switch (methodId) {
    case "heuristic-evaluation":
      return `Apply Jakob Nielsen's 10 Usability Heuristics:
...
Include the specific heuristic violated, the location in the UI, and a concrete recommendation.`;
    ...
    case "cognitive-walkthrough":
      return `For each task step, evaluate these four questions:
...
Mark each step as PASS or FAIL per question. Identify confusion points and provide specific remediation.`;
    ...
    case "accessibility-audit":
      return `Evaluate against WCAG 2.1 AA guidelines across these categories:
...
Rate issues by conformance level (A, AA, AAA) and impact severity.`;
    ...
    case "ia-review":
      return `Evaluate Information Architecture across:
...
Use card sorting principles to suggest restructuring where needed.`;
    ...
    case "usability-test-plan":
      return `Create a complete test plan including:
...
- Analysis framework and reporting template`;
    ...
    case "ux-metrics-framework":
      return `Define metrics using the HEART framework (Happiness, Engagement, Adoption, Retention, Task success) plus:
...
- Dashboard structure: How to visualize and track over time`;
    ...
    case "persona-development":
      return `Create 3-5 distinct user personas, each including:
...
Base personas on the target audience data provided. Make them specific and realistic, not generic archetypes.`;
    ...
    case "user-journey-mapping":
      return `Map the journey across these stages:
...
For each stage, document: user actions, thoughts, emotions (positive/negative), pain points, opportunities, and touchpoints.`;
    ...
    case "competitive-ux-analysis":
      return `Analyze competitors across these dimensions:
...
Conclude with strategic recommendations for differentiation.`;
    ...
    case "task-analysis":
      return `Use Hierarchical Task Analysis (HTA) to decompose tasks:
...
- Recommend optimizations with estimated step reduction`;
    ...
    case "research-plan":
      return `Create a comprehensive research plan including:
...
- Deliverables and reporting schedule`;
    ...
    case "activity-protocol":
      return `Create a detailed session protocol including:
...
- Estimated timing per section`;
    ...
    case "survey-analysis":
      return `Analyze the provided survey data:
...
- Recommended follow-up: Questions for deeper investigation`;
    ...
    case "interview-analysis":
      return `Analyze the provided interview/session data:
...
- Actionable recommendations: Prioritized by impact and frequency`;
    ...
    case "content-audit":
      return `Audit the provided content across:
...
- Prioritized recommendations: What to fix, consolidate, or create`;
    ...
    default:
      return `Follow established UX research best practices for this method. Structure your output clearly with headings and actionable findings.`;
  }
}
